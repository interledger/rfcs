...
A single byte in the range 128 .. 255, denoting the value _128 + n_, where _n_ is the length-of-length. This is followed by an unsigned integer of _n_ bytes denoting the length. This length MUST NOT contain leading zeros. If the length is less than 127, the short form encoding MUST be used.
...
When encoded in binary, the shortened date string is encoded as a fixed-length string.
