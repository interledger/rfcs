...
In addition, TLS Client Certificates are RECOMMENDED between peers for additional security. Basic authentication (username and password) is NOT RECOMMENDED for the same reasons that the `SIMPLE` profile is only reccommended for development and testing scenarios only.
...
##### Example Usage
An example shared-secret in this profiles is `HEiMCp0FoAC903QHueY89gAWJHo/izaBnJU8/58rlSI=`. This shared secret is passed as an `Authorization` header in each HTTP request, using the Bearer token scheme, along with an `Auth-Principal` header like this: 
