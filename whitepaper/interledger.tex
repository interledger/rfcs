...
relationship between two agents is defined by a unit of value, and a log of HTLAs, whose amounts (expressed in the relationship's unit of value)
...
HTLA's hash. Apart from this destination field, the packets that describe Interledger transfers have {\tt data} fields, which should be transported end-to-end over a multi-hop chain (see below).
...
to use the preimage presented by Charlie, to fulfill the incoming transfer from Alice before its deadline. We call this a two-hop payment: from Alice, via Bob, to Charlie.
...
network topology.
In any case, the first two agents to form a new network can choose their own identifiers freely, as long as they are different from each other. When a new agent joins, one of two things can happen:
...
in that order (see https://github.com/interledger/rfcs/blob/master/asn1/InterledgerProtocol.asn#L16-L31).
...
{\tt triggeredBy} (}IA5String (size 1..1023)}, the ILP address of the agent who triggered this rejection),
{\tt message} (}Ia5String (size 0..8191)}, a human-readable error message),
and {\tt data} (to be passed back unaltered from receiver to sender, an {\tt OCTET STRING (size 0..32767)}, in that order.
